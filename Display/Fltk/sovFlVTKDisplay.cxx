...
//---------------------------------------------------------------------------
FlVTKDisplay::FlVTKDisplay( int x, int y, int w, int h, const char *l ) : 
...
#ifdef VTK_DEBUG_LEAKS
    vtkDebugLeaks::DestructClass("FlVTKDisplay");
#endif
...
//---------------------------------------------------------------------------
FlVTKDisplay * FlVTKDisplay::New()
{
    // we don't make use of the objectfactory, because we're not registered
    return new FlVTKDisplay;
}

//---------------------------------------------------------------------------
void FlVTKDisplay::Initialize()
{
    // if don't have render window then we can't do anything yet
    if (!RenderWindow)
    {
    vtkErrorMacro(<< "FlVTKDisplay::Initialize has no render window");
    return;
    }
...
//---------------------------------------------------------------------------
void FlVTKDisplay::Start()
{
    // the interactor cannot control the event loop
    vtkErrorMacro(<<"FlVTKDisplay::Start() interactor cannot control event loop.");
}
//---------------------------------------------------------------------------
void FlVTKDisplay::SetRenderWindow(vtkRenderWindow *aren)
{
   vtkRenderWindowInteractor::SetRenderWindow(aren);
   // if a FlVTKDisplay has been shown already, and one
...
void FlVTKDisplay::UpdateSize(int W, int H)
{
...
{
    if (p)
      ((FlVTKDisplay *)p)->OnTimer();
}

//---------------------------------------------------------------------------
int FlVTKDisplay::CreateTimer(int timertype)
{
...
//---------------------------------------------------------------------------
int FlVTKDisplay::DestroyTimer()
{
    // do nothing
    return 1;
}

void FlVTKDisplay::OnTimer(void)
{
    if (!Enabled)
      return;
...
#if ((VTK_MAJOR_VERSION == 4 && VTK_MINOR_VERSION > 0) || (VTK_MAJOR_VERSION > 4))
    // new style
...
//---------------------------------------------------------------------------
void FlVTKDisplay::TerminateApp()
{
    ;
}
...
//---------------------------------------------------------------------------
void FlVTKDisplay::resize( int x, int y, int w, int h ) {
...
int FlVTKDisplay::handle( int event ) {
    if( !Enabled ) return 0;
    ...
    break;
      ...
      {
      m_KeyboardInteractor->Shortcut(Fl::event_key());
      }
#if ((VTK_MAJOR_VERSION == 4 && VTK_MINOR_VERSION > 0) || (VTK_MAJOR_VERSION > 4))      
        // new style
    ...
    switch( Fl::event_button() ) 
    {
      case FL_LEFT_MOUSE:
        ...
        if(m_MouseInteractor)
          {
          m_MouseInteractor->Push(Fl::event_x(),this->h()-Fl::event_y()-1,0);
          }
        break;
...
static char const rcsid[] =
  "$Id: sovFlVTKDisplay.cxx,v 1.3 2006-02-28 00:11:53 jjomier Exp $";

const char *FlVTKDisplay_rcsid(void)
{
    return rcsid;
}
...
::AddRenderer( RendererType * renderer )
{
  ...
  this->RenderWindow->AddRenderer(static_cast<VTKRenderer<3>*>(renderer)->GetVTKRenderer());
